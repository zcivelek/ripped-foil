...
**The null and full models are not significantly different from each other. But the interaction term is not significant. I will remove this next to see if it improves the model.**
...
**The model shows a significant effect of trial type**
...
round(xdrop1,3)
```
**The model without the interaction is not significantly different from the null model either.**
**Question: Since the model is not a good fit to the data, I cannot talk about the significant effect of trial type here, is this correct?**
...
#Test phase
wt1<-better.wilcox(conmon_test$correct) 
wt1
median = round(median(conmon_test$correct), 2)
median 

wt2<-better.wilcox(conmon_transfer$correct) 
wt2
median2 = round(median(conmon_transfer$correct), 2)
median2 
```
**Neither test nor transfer performances are different from chance. Any idea why I get wrong sample sizes here?**

##Mann-Whitney U-Test  

```{r, echo=FALSE}
mt<-wilcox.exact(conmon_separate$correct~conmon_separate$phase,exact=T)
mt
...
u_mt<-min(c(mt$statistic, prod(table(conmon_separate$phase))-mt$statistic))
u_mt
#to get the sample sizes use:
n1_mt<-table(conmon_separate$phase)[1]
n2_mt<-table(conmon_separate$phase)[2]
n1_mt
n2_mt
```
**There is no signficiant difference between  the test and transfer performances either.**


```{r, include=FALSE}
#this is for normally distributed samples. Mine is not but I'll keep it for future.
...
```{r, include=FALSE}
#It doesn't make sense to explore trial type because the model was not significant. and even if I did, I should use wilcoxon test as the sample is not normally distributed. But keeping this for the future.
...
```{r, include=FALSE}
#This is trialtype graph. Include it only if it makes sense to further analyze this. Keeping it for the future.
