...
```{r}
source("C:/Users/Zeynep/Desktop/ripped-foil/Ripped Foil/Roger_functions/diagnostic_fcns.r")
source("C:/Users/Zeynep/Desktop/ripped-foil/Ripped Foil/Roger_functions/glmm_stability.r")
source("C:/Users/Zeynep/Desktop/ripped-foil/Ripped Foil/Roger_functions/boot_glmm.r")
```
...
boot.res=boot.glmm.pred(model.res=full.CI, excl.warnings=T, nboots=1000, resol=68, level=0.95, use="agemonths")
...
Attempt at getting the predictions and CI to the plot for agemonths ~ correct

```{r}
correctBYage<-ggplot(ripchild_individual, aes(agemonths, correct), ylim=c(0,1))
s1<-correctBYage + geom_point(aes(), position="jitter") + labs(x= "Age in months", y= "Mean Performance in Test and Transfer")+ 
...
lines(plot.agemonths, boot.res$ci.predicted$fitted, lty=2)
lines(plot.agemonths, boot.res$ci.predicted$lower.cl, lty=3)
lines(plot.agemonths, boot.res$ci.predicted$upper.cl, lty=3)
```
...
ggplot(ripchild_individual, aes(x=plot.agemonths, y=boot.res$ci.predicted$fitted)) +
  geom_jitter(aes(x=agemonths, y=correct))+
  geom_ribbon(data=boot.res$ci.predicted, aes(x = plot.agemonths, ymin=boot.res$ci.predicted$lower.cl,
...
Next things to do: Find effect sizes for the full model- calculation at Roger
There was no effect of phase so don't look for the first trial of transfer anymore. If yes, table from Manuel's paper with number of kids who got trial 1 right and the p's from binom.

```{r}
plot_model(full2, type="est") 
```
