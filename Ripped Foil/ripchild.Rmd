...
openended$explanation<-as.factor(openended$explanation)
...
nullage<- glmer(correct ~ 1 + z.trialno + (1|id), data=agematched, family=binomial, control=contr)
round(anova(agecomp, nullage, test="Chisq"),3)

correctexplanation <-agematched%>%
  filter(explanation == "1")
testcorrect <- glmer(correct ~ 1 + (1 | id), data= correctexplanation, family=binomial)
summary(testcorrect)

incorrectexplanation <-agematched%>%
  filter(explanation == "0")
testincorrect <- glmer(correct ~ 1 + (1| id),data= incorrectexplanation, family=binomial)
