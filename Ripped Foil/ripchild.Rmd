...
Next is the pre-processing of data (z-transformations for 'trial no' and 'age in months'; checking if factors are correctly identified, coding dummy variables for trialtype, sex and phase (order3), and fially centering the random slope components for these variables.
...
```{r}
boot.res=boot.glmm.pred(model.res=full2, excl.warnings=T, nboots=1000, resol=68,
level=0.95, use="z.age")

boot.res$ci.estimates
```

Attempt at getting the predictions and CI to the plot for age ~ correct
...
Next, I'll look at the distribution of children's performances in Test and Transfer phases across age groups.
