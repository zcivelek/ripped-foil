...
plot.agemonths=seq(from=min(ripchild_individual2$z.age), to=max(ripchild_individual2$z.age),length.out=68)

scatter<-ggplot(ripchild_individual2, aes(x=plot.agemonths, y=boot.res$ci.predicted$fitted)) +
  geom_point(aes(x=z.age, y=correct), size=ripchild_individual2$n, alpha=0.8)+ geom_ribbon(data=boot.res$ci.predicted, aes(x =z.age,ymin=boot.res$ci.predicted$lower.cl,ymax=boot.res$ci.predicted$upper.cl), fill="grey", alpha=0.5)+
...
plot.agemonths=seq(from=min(ripchild_individual2$z.age),to=max(ripchild_individual2$z.age),length.out=68)
plot(plot.agemonths)
lines(x=plot.agemonths, y=boot.res$ci.predicted$fitted,lty=2)
lines(x=plot.agemonths, y=boot.res$ci.predicted$lower.cl,lty=3)
lines(x=plot.agemonts, y=boot.res$ci.predicted$upper.cl,lty=3)
?lines()
```

```{r}
#trying something else
attributes(boot.res$ci.predicted$z.age)
install.packages("DMwR")
library(DMwR)
unsc.zage <- unscale(boot.res$ci.predicted, boot.res$ci.predicted$z.age)

#didn't work. another attempt
...
unscale(boot.res$ci.predicted$z.age)

#another attempt!!!

age.orig = t(lapply(boot.res$ci.predicted$z.age, function(r)r*attr(boot.res$ci.predicted$z.age,'scaled:scale') + attr(boot.res$ci.predicted$z.age, 'scaled:center')))

# another attempt

boot.res$ci.predicted$z.age * attr(boot.res$ci.predicted$z.age, 'scaled:scale') + attr(boot.res$ci.predicted$z.age, 'scaled:center')
...
unnormalize <- function(x, rec, var) {
  ...
  (x * var_sd) + var_mean}
unnormalize(boot.res$ci.predicted,boot.res$ci.predicted$z.age)

boot.res$ci.predicted

#last attempt
ripchild_individual2$z.age * attr(ripchild_individual2$z.age, 'scaled:scale') + attr(ripchild_individual2$z.age, 'scaled:center')
...
scatter.int<-ggplot(ripchild_separate2, aes(x=z.age, y=boot.res.int$ci.predicted$fitted)) +
  facet_wrap(~order3, nrow = 2)+
  geom_point(aes(x=z.age, y=correct, fill=order3))
  
scatter.int

#up to here it works... I need to find a way to add confidence intervals separately to two plots.
#the rest of the code: geom_ribbon(data=boot.res.int$ci.predicted, aes(x =z.age,ymin=boot.res.int$ci.predicted$lower.cl,ymax=boot.res.int$ci.predicted$upper.cl), fill="grey", alpha=0.5)+
  geom_line(data=boot.res.int$ci.predicted, aes(x = z.age,y=boot.res.int$ci.predicted$fitted), lty=2)+
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ xlab("Age in months")+ ylab("Mean correct responses in Test and Transfer")+ 
  #xlim(36,72)+
  ylim(0,1)+
  geom_hline(yintercept=0.50, color='red', linetype="dashed")
...
```{r}
firsttrial <- glm(correct~agemonths,data=ripchild_1st_trial,family=binomial(link = "logit"))
summary(firsttrial)

newdat <- data.frame(agemonths=seq(min(ripchild_1st_trial$agemonths), max(ripchild_1st_trial$agemonths),len=68))
newdat$correct = predict(firsttrial, newdata=newdat, type="response")
...
```{r}
plot(correct~agemonths, data=ripchild_1st_trial)
lines(correct~agemonths, newdat, col="blue", lwd=2)
#taken from: https://stackoverflow.com/questions/36685921/plot-logistic-regression-curve-in-r/36686468
```

```{r}
#how about ggplot
ggplot(ripchild_1st_trial, aes(x=agemonths, y=correct)) + geom_point() + 
  stat_smooth(method="glm", fullrange = TRUE, method.args=list(family="binomial"))+ylab("First trial performance of Transfer Phase")+xlab("Age in months")+xlim(36,72)
#add abline to this
```
...
#write_csv(mtcars, path = "mtcars.csv") <- to save the data in csv
```
