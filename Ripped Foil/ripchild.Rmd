...
firsttrial <- glm(correct~z.age+sex+trialtype,data=ripchild_1st_trial,family=binomial(link = "logit"))
summary(firsttrial)
anova(firsttrial, test="Chisq")
...
ggplot(openended, aes(x=agemonths, y=explanation)) + geom_point() + 
  stat_smooth(method="glm", fullrange = TRUE, lty=2, size=0.5, colour="Black", alpha=0.2,  method.args=list(family="binomial"))+ylab("Incorrect (0)/correct (1) explanations")+xlab("Age in months")+xlim(36,72)+theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```
