...
```{r, echo=TRUE}
null2<-glmer(correct~ (1+z.trialno+phase.transfer.c+trialtype.food.c|id),data=ripmon1, family=binomial, control=contr)
```
...
ttransfer
```
**Performance is above chance in both Test (last 16 trials) and Transfer phases.**
...
ttype
```
**There is a significant difference between performances in different trial types: Monkeys performed better in food-stick trials than in stick-food trials.**
