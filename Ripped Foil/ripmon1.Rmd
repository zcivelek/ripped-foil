...
```{r}
#Amanda requested to see how monkeys' performance changed throughout trials of Test and Transfer phase (faceted)- to show visually that performance gets better over time.

###First do all the transformations 


ripmon3<-read.csv("ripped_monkey_complete_testtransfer.csv", header=T)
str(ripmon3)
...
ripmon3$sessiontype.transfer=as.numeric(ripmon3$sessiontype==levels(ripmon3$sessiontype)[2])

#centering the slopes: p-values of the factors can be influenced by the choice of reference category.
#by centering the factor for the random slope components the p-values should be the same irrespective of the choice of the reference level
ripmon3$trialtype.food.c<-ripmon3$trialtype.food-mean(ripmon3$trialtype.food)
ripmon3$sessiontype.transfer.c=ripmon3$sessiontype.transfer-mean(ripmon3$sessiontype.transfer)
...
full.CI.int <- glmer(correct ~ trialtype.food.c + z.age + sex.m.c + z.trialno*sessiontype+ (1|id)+(0+z.trialno+sessiontype.transfer.c+trialtype.food.c|id), data=ripmon3, family=binomial, control=contr)

boot.res.int=boot.glmm.pred(model.res=full.CI.int, excl.warnings=T, nboots=5, resol=80, level=0.95, use="z.trialno", n.cores="all-1", para=T)
...
scatter.int2 <- ggplot(ci.data, aes(y = fitted)) +
  ...
  geom_ribbon(data = ci.data%>%filter(sessiontype=="test"), aes(x = plot.int, y = fitted, ymin = lower.cl, ymax = upper.cl, group = sessiontype), fill = "grey", alpha = 0.5) +
  geom_line(data = ci.data%>%filter(sessiontype=="test"), aes(x = plot.int, y = fitted), lty = 2) +
    geom_ribbon(data = ci.data%>%filter(sessiontype=="transfer"), aes(x = plot.int, y = fitted, ymin = lower.cl, ymax = upper.cl, group = sessiontype), fill = "grey", alpha = 0.5) +
  geom_line(data = ci.data%>%filter(sessiontype=="transfer"), aes(x = plot.int, y = fitted), lty = 2) + scale_x_continuous(breaks = seq(from = 1, to = 80, by = 8)) +
  theme_bw() +
