...
ripmon3$phase.transfer=as.numeric(ripmon3$phase==levels(ripmon3$phase)[2])

#centering the slopes: p-values of the factors can be influenced by the choice of reference category.
#by centering the factor for the random slope components the p-values should be the same irrespective of the choice of the reference level
ripmon3$trialtype.food.c<-ripmon3$trialtype.food-mean(ripmon3$trialtype.food)
ripmon3$phase.transfer.c=ripmon3$phase.transfer-mean(ripmon3$phase.transfer)
...
full.CI.int <- glmer(correct ~ trialtype.food.c + z.age + sex.m.c + z.trialno*phase+(1|id)+(0+z.trialno+phase.transfer.c+trialtype.food.c|id), data=ripmon3, family=binomial, control=contr)
```
