...
Next we preprocess the data; scaling age and trial number, coding categorical variables as dummy variables and centering the slopes.
...
Full model (removed box location from random effects)

```{r}
contr<-glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=10000000))
full=glmer(correct ~ trialtype*phase+z.trialno+z.age+sex+(1+z.trialno+phase.transfer.c+trialtype.food.c|id),data=ripmon1, family=binomial, control=contr)
```
...
```{r}
null=glmer(correct~(1+z.trialno+phase.transfer.c+trialtype.food.c|id),data=ripmon1, family=binomial, control=contr)
```
...
```{r}
full2=glmer(correct ~ trialtype+phase+z.trialno+z.age+sex+(1+z.trialno+phase.transfer.c+trialtype.food.c|id),data=ripmon1, family=binomial, control=contr)
summary(full2)
```

I get the following warning: convergence code: 0, boundary (singular) fit: see ?isSingular.

The results show that there is a significant effect of sex and trial type.
...
Looking at the graph before running the analyses on the effect of sex and trial type
...
p1 <- ggplot(data=ripmon_individual2, aes(x=phase, y=foil.correct, group=phase)) + geom_boxplot()+ylim(0,1)+  geom_point(size = ripmon_individual2$n, colour = "darkgrey", alpha=0.3) +labs(x="",y="Mean number of correct choices")+theme_few()+ggtitle("Phase")+geom_hline(yintercept=0.5, linetype="dashed", color = "red")
...
Performance in test and transfer with a bar graph.
...
```{r, echo=FALSE}
ttest<-t.test(ripmon_separate$correct[ripmon_separate$phase=="test"] , mu=0.5, alternative = "two.sided")
ttest
ttransfer<-t.test(ripmon_separate$correct[ripmon_separate$phase=="transfer"] , mu=0.5, alternative = "two.sided")
ttransfer
```
...
```{r}
comptt<-t.test(ripmon_separate$correct[ripmon_separate$phase=="test"], ripmon_separate$correct[ripmon_separate$phase=="transfer"], paired = TRUE, alternative = "two.sided")
comptt
```
...
```{r, echo=FALSE}
ttypefood<-t.test(ripmon_individual3$foil.correct[ripmon_individual3$trialtype=="food"] , mu=0.5, alternative = "two.sided")
ttypefood
ttypestick<-t.test(ripmon_individual3$foil.correct[ripmon_individual3$trialtype=="stick"], mu=0.5, alternative="two.sided")
ttypestick
```

####Comparison between food-stick, stick-food trials

```{r}
ttype<-t.test(ripmon_individual3$foil.correct[ripmon_individual3$trialtype=="food"], ripmon_individual3$foil.correct[ripmon_individual3$trialtype=="stick"], paired = TRUE, alternative = "two.sided")
ttype
```
...
```{r}
ttmale<-t.test(ripmon_individual4$foil.correct[ripmon_individual4$sex=="m"] , mu=0.5, alternative = "two.sided")
ttmale
ttfemale<-t.test(ripmon_individual4$foil.correct[ripmon_individual4$sex=="f"] , mu=0.5, alternative = "two.sided")
ttfemale
```
####Comparison between males and females

```{r}
tsex<-t.test(ripmon_individual4$foil.correct[ripmon_individual4$sex=="m"], ripmon_individual4$foil.correct[ripmon_individual4$sex=="f"], paired = FALSE, alternative = "two.sided")
tsex
```
...
ripmon_1st_trial <- ripmon1 %>%
  ...
  group_by(id, age, sex, trialtype, order) %>% 
  summarize(correct)
```
...
```{r}
firsttrial <- glm(correct~age+sex+trialtype,data=ripmon_1st_trial,family=binomial(link = "logit"))
summary(firsttrial)
```
...
How about the first 8 trials of Test phase
